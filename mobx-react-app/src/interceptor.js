...
const isAccessTokenExpired = (headers) => {
  if (
    Utility.stringToBoolean(headers["access-token-expired"]) ||
    Utility.stringToBoolean(headers["invalid-token"])
  ) {
    return true;
  }
  return false;
...
http.interceptors.request.use(
  (request) => {
    ...
    if (checkToken(request.url)) {
      request.headers.authorization = `Bearer ${Utility.getToken(
        "accessToken"
      )}`; // eslint-disable-line
    }
